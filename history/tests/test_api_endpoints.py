...
class TestAPIEndpoints:
    ...
    class TestWeatherDayListView:
        def test_pagination_works(self, client):
            ...
            assert response1.data['results'][0] == {'station': {'code': 'file_to_load'}, 'date': '1989-03-13', 'temperature_max': 122, 'temperature_min': -44, 'precipitation': None}
            assert response1.data['results'][1] == {'station': {'code': 'file_to_load'}, 'date': '1989-03-14', 'temperature_max': 189, 'temperature_min': None, 'precipitation': 53}

            assert response2.data['count'] == 6
            assert len(response2.data['results']) == 2
            assert response2.data['results'][0] == {'station': {'code': 'file_to_load'}, 'date': '1989-03-15', 'temperature_max': None, 'temperature_min': 6, 'precipitation': 0}
            assert response2.data['results'][1] == {'station': {'code': 'file_to_load'}, 'date': '1989-03-16', 'temperature_max': 94, 'temperature_min': -33, 'precipitation': 0}
        ...
        def test_pagination_and_filters_work_together(self, client):
            ...
            assert response.data['results'][0] == {'station': {'code': 'file_to_load'}, 'date': '1989-03-15', 'temperature_max': None, 'temperature_min': 6, 'precipitation': 0}
            assert response.data['results'][1] == {'station': {'code': 'file_to_load'}, 'date': '1989-03-16', 'temperature_max': 94, 'temperature_min': -33, 'precipitation': 0}
    ...
    class TestWeatherStatsListView:
        ...
        def test_pagination_and_filters_work_together(self, client):
            response = client.get('/api/weather/stats?station__code=file_to_load2&year=1989')
